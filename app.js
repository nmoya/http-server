...
function routeRequests() {
  ...
  app.post("/whosintoyou/receivedata", function(req, res) {
    if (!req.body) {
    ...
    root = req.body.name;
    console.log("Processing request from: " + root);
    ...
    fs.writeFile(in_file_name, chat, function(write_error) {
      ...
      cmd = util.format("python ./whosintoyou/whatsapp-parser/Chat.py -f %s -p Facebook -o %s -n %s", in_file_name, out_file_name, root.replace(" ", "_"));
      console.log(cmd);
